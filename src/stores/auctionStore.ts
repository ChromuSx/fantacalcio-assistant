...
export const useAuctionStore = create<AuctionStore>()(
  persist(
    (set, get) => ({
      ...
      // Acquisto giocatore
      purchasePlayer: (player, price, owner = "me") =>
        set((state) => {
          ...
          if (owner === "me") {
            return {
              ...
              myTeam: [
                ...state.myTeam,
                { ...player, status: "mine" as PlayerStatus, paidPrice: price },
              ],
      ...
      generateNotesForPlayer: (playerId) => {
        ...
        set((state) => ({
          notes: [
            ...state.notes.filter(
              (n) => n.playerId !== playerId && n.type === "manual"
            ),
            ...newNotes,
          ],
        }));
      ...
      generateAllNotes: () => {
        ...
        const allNotes = players.flatMap((p) =>
          NotesGenerator.generateNotes(p)
        );
      ...
      getSlotsInfo: () => {
        ...
        Object.entries(state.config.POSTI_RUOLO).forEach(([ruolo, posti]) => {
          const taken = state.myTeam.filter(
            (p) => p.ruolo === (ruolo as Role)
          ).length;
          info[ruolo as Role] = {
            taken,
            total: posti,
      ...
      getAvailablePlayers: (role) => {
        ...
        return state.players.filter(
          (p) => p.status === "available" && (!role || p.ruolo === role)
        );
      },
      ...
      calculateMaxPrice: (player) => {
        ...
        // Considera i range di gol/assist se disponibili
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state && state.players) {
          state.players = state.players.filter(
            (p) => p.ruolo && ["P", "D", "C", "A"].includes(p.ruolo)
          );
        }
